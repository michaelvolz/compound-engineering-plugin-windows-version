...
If not installed, inform the user: "`agent-browser` is not installed. Run `skill({ name: "ce-setup" })` to install required dependencies." Then stop — this skill cannot function without agent-browser.
...
If not installed, inform the user: "`agent-browser` is not installed. Run `skill({ name: "ce-setup" })` to install required dependencies." Then stop.
