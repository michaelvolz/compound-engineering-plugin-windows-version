...
```bash
mkdir -p .context/compound-engineeringskill({ name: "ce-optimize" })/<spec-name>/
```
...
rm -f .context/compound-engineeringskill({ name: "ce-optimize" })/<spec-name>/strategy-digest.md
```
