...
**If the user chooses Full**, ask one follow-up question before proceeding. Detect which harness is running (Claude Code, OpenCode, Codex, or Cursor) and ask:
...
#### 4. **Session Historian** (foreground, after launching the above — only if the user opted in)
   ...
   - Dispatch in **foreground** — this agent reads session files outside the working directory (`~/.claude/projects/`, `~/.local/share/opencode/`, `~/.codex/sessions/`, `~/.cursor/projects/`) which background agents may not have access to
   - Searches prior Claude Code, OpenCode, Codex, and Cursor sessions for the same project to find related investigation context
...
<manual_override> Use skill({ name: "ce-compound" }) [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
